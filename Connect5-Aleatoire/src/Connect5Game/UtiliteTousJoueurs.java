...
public class UtiliteTousJoueurs implements Utilite {
    ...
    private int[] calculerUtilite(Grille grille) {
        ...
        for (int l = 0; l < grille.data.length; ++l) {
            for (int c = 0; c < grille.data[0].length; ++c) verifierCase(grille.data[l][c]);
            verifierCase(20);
        }

        // Verticale
        for (int c = 0; c < grille.data[0].length; ++c) {
            for (int l = 0; l < grille.data.length; ++l) verifierCase(grille.data[l][c]);
            verifierCase(20);
        }
        ...
        for (int c = -grille.data.length; c < grille.data[0].length; ++c) {
            ...
            for (; c2 < grille.data[0].length && l >= 0; ++c2, --l) verifierCase(grille.data[l][c2]);
            verifierCase(20);
        }
    ...
    private void verifierCase(int value) {
        if (value != lastValue) {
            ...
            if (estJoueur(value)) ++count;
            else if (value == 0) ++suiteZero;
            lastValue = value;
        }
    }
